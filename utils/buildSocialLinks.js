// utils/buildSocialLinks.js
//
// One source of truth for the footer social icons.
//
// The six templates each carried their own copy of this markup, which drifted:
// the legal pages had four icons, the rest had five, and none had Instagram
// even though the form collects it.
//
// It also renders ONLY the platforms that have a URL. Previously an unset
// platform still emitted <a href=""> with its icon — a visible link that
...
const PLATFORMS = [
  ...
  /** Treat '#', 'n/a' and blanks as "not provided". */
  ...
  /**
   * Footer social icons for whichever platforms have a URL.
   * Returns '' when none are set, so the row simply stays empty.
   */
  function buildSocialLinks(globalValues = {}) {
    return PLATFORMS
      .map(p => {
        const url = usableUrl(globalValues[p.key]);
        if (!url) return '';
  
        return `
            <a href="${escapeAttr(url)}" target="_blank" aria-label="${p.label}" rel="noopener noreferrer">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="${p.fill}" viewBox="${p.viewBox}" aria-label="${p.label}">
                <path d="${p.path}"/>
              </svg>
            </a>`;
      })
  ...
  module.exports = { buildSocialLinks, PLATFORMS };