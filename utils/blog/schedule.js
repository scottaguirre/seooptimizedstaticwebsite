...
function publishDates({ count, everyDays = 7, publishTime = '09:00', timezone = 'UTC', startAt }) {
  ...
  let base = zonedDateParts(anchor, zone);
  ...
  const runFrom = (from) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const stepped = new Date(Date.UTC(from.year, from.month - 1, from.day + i * gap));
  ...
  //
  // THIS USED TO RECURSE, AND IT WAS A REAL BUG. The recursive call passed a
  // UTC-MIDNIGHT Date as the new startAt, and the first thing the next call
  // did was read that instant back with zonedDateParts. In any zone behind
  // UTC — every American one — midnight UTC on the 6th is still the evening
  // of the 5th, so the anchor came back as the SAME calendar day it started
  // from. With everyDays: 7 that still crept forward six days a time and
  // terminated; with everyDays: 1 it advanced by nothing at all and recursed
  // until the stack ran out, which is what a customer saw as "Could not
  // create the campaign".
  //
  // So the anchor is now carried as plain calendar numbers and never turned
  // back into an instant to be re-interpreted. One shift is always enough —
  // the next slot is at least a calendar day past a time that is at most 24
  // hours behind now — but the loop is capped rather than trusted, so a zone
  // doing something genuinely strange costs a few iterations instead of the
  // process.
  for (let guard = 0; guard < 8 && out.length && out[0].getTime() <= Date.now(); guard += 1) {
    const shifted = new Date(Date.UTC(base.year, base.month - 1, base.day + gap));
    base = {
