...
const buildservicesNavMenu = function (pages, basePath, locationSlug, filename){
    // ===  NavMenu =====
    let linkOutsideNavMenu = "";
    let firstPageName = "";
    let firstPageNameActive = "";

    const servicesNavMenu = Object.entries(pages).map(([i, p]) => {
        ...
        if(Object.entries(pages).length > 1){ // If 2 pages or more are entered
            ...
            linkOutsideNavMenu = `${basePath}${pageSlug}-${normalizeText(locationSlug)}.html`;
            firstPageName = p.filename.toUpperCase();
            firstPageNameActive = isActive;
            return  "";
        }
...
module.exports = { buildservicesNavMenu };
